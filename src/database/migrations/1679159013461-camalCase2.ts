...
export class camalCase21679159013461 implements MigrationInterface {
    name = 'camalCase21679159013461'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`CommentImage\` (\`id\` int NOT NULL AUTO_INCREMENT, \`articleId\` int NOT NULL, \`writerId\` int NOT NULL, \`content\` varchar(255) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`commentImage\` ADD CONSTRAINT \`FK_53b3b9a7aae87532a533fd3e0eb\` FOREIGN KEY (\`commentId\`) REFERENCES \`CommentImage\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`CommentImage\` ADD CONSTRAINT \`FK_e6159ce881e1693093ae6d72430\` FOREIGN KEY (\`articleId\`) REFERENCES \`article\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`CommentImage\` ADD CONSTRAINT \`FK_25c0009a3c72d99f79c354135cf\` FOREIGN KEY (\`writerId\`) REFERENCES \`user\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`CommentImage\` DROP FOREIGN KEY \`FK_25c0009a3c72d99f79c354135cf\``);
        await queryRunner.query(`ALTER TABLE \`CommentImage\` DROP FOREIGN KEY \`FK_e6159ce881e1693093ae6d72430\``);
