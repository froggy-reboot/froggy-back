  ...
  async userSignUp(mailData: MailData<{ hash: string }>) {
    await this.mailerService.sendMail({
      ...
      text: `${this.configService.get(
        'app.backendDomain',
      )}/api/v1/auth/email/confirm/${mailData.data.hash} ${await this.i18n.t(
        'common.confirmEmail',
      )}`,
      template: 'activation',
      context: {
        ...
        url: `${this.configService.get(
          'app.backendDomain',
        )}/api/v1/auth/email/confirm/${mailData.data.hash}`,
  ...
  async forgotPassword(mailData: MailData<{ hash: string }>) {
    await this.mailerService.sendMail({
      ...
      context: {
        ...
        url: `${this.configService.get(
          'app.backendDomain',
        )}/api/v1/auth/reset-password/${mailData.data.hash}`,
